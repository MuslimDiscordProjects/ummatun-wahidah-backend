...
async function editProject(name: string, meta?: Object, image?: string): Promise<Project | null> {
    ...
    if (meta !== undefined) {
        let _meta: any = meta;
        if (_meta.sources !== undefined) _meta.sources = `${_meta.sources}`;
        if (_meta.links !== undefined) _meta.links = `${_meta.links}`;
        if (_meta.content !== undefined) _meta.content = `${_meta.content}`;
        ProjectObject.project_meta = JSON.stringify(_meta);
    };
